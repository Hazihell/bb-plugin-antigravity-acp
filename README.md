...
## Approvals in agent-started turns

bb treats a turn that another thread or the system started (a `bb thread tell`
from a coordinator, for example) as one that may not escalate approvals to a
human, and auto-denies approval requests for bridges that leave enforcement to
the runtime. Antigravity remembers the first deny as a session-wide wildcard
rule, so one steered turn would leave the session unable to run commands or
read files. This plugin reports its bridge as enforcing approvals itself (the
ACP bridge already allows in place under full permission mode and asks
otherwise), so those requests are raised as normal approvals instead.

## Wire trace

Create the directory `~/.gemini/antigravity-acp/trace` and the POSIX wrapper
appends every JSON-RPC line it proxies, in both directions, to
`<pid>.jsonl` inside it. Remove the directory to stop tracing.
