...
const { appendFileSync, existsSync, realpathSync } = require("node:fs");
...
function main() {
  ...
  // Opt-in wire trace: when \`<geminiHome>/antigravity-acp/trace\` exists,
  // every JSON-RPC line crossing the proxy is appended to
  // \`<that dir>/<pid>.jsonl\` with its direction. Diagnostics only.
  const traceDir = join(geminiHome, "antigravity-acp", "trace");
  const tracePath = existsSync(traceDir) ? join(traceDir, String(process.pid) + ".jsonl") : null;
  const trace = (direction, data) => {
    if (!tracePath) return;
    try {
      appendFileSync(tracePath, JSON.stringify({ t: Date.now(), direction, line: String(data).replace(/\\n$/, "") }) + "\\n");
    } catch {}
  };
  ...
  const pipeLine = (dest, rl, data) => {
    ...
    trace(dest === process.stdout ? "to-client" : "to-agent", data);
